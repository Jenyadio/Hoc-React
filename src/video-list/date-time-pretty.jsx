...
import {getDayCaption, getHoursCaption} from "./utils/utils";
...
function dateOperator(value) {
    ...
    if (difHours < 1) return 'меньше часа назад';
    if (difHours < 24) return `${difHours} ${getHoursCaption(difHours)} назад`;
...
function wrapper(InitialComponent, operator) {
    return class extends React.Component {
        ...
        render() {
            return (<InitialComponent {...this.props} date={this.state.date}/>)
        }
    }
}
